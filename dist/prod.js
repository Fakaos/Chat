...
var MemStorage = class {
  ...
  async getErrors(limit = 50) {
    return this.logs.filter((log) => log.level === "error" && !log.message.toLowerCase().includes("invalid credentials") && !log.message.toLowerCase().includes("login failed")).slice(-limit).reverse();
  }
  async getNgrokUrl() {
...
app.use((req, res, next) => {
  ...
  res.on("finish", () => {
    ...
    if (path2.startsWith("/api") && process.env.NODE_ENV === "development") {
      console.log(`[express] ${req.method} ${path2} ${res.statusCode} in ${duration}ms`);
    }
  });
  next();
});
